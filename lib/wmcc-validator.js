...
class Validator {
   ...
   * @constructor
   * @param {Object} map
   * @param {Options} [Bool:loose, Fn:errorHandler]
   */
  constructor(map, options = {}) {
    ...
    this.map = map;
    this.loose = options.loose || false;
    this._errHandler = options.errorHandler || ValidationError;
  }
   ...
   */
  static multi(maps, options) {
    return new MultiValidator(maps, options);
  }
  ...
  str(key, fallback) {
    ...
    if (typeof value !== 'string')
      throw new this._errHandler(key, 'string');
  ...
  int(key, fallback) {
    ...
    if (typeof value !== 'string') {
      if (typeof value !== 'number')
        throw new this._errHandler(key, 'integer');
  ...
  uint(key, fallback) {
    ...
    if (value < 0)
      throw new this._errHandler(key, 'unsigned integer');
  ...
  float(key, fallback) {
    ...
    if (typeof value !== 'string') {
      if (typeof value !== 'number')
        throw new this._errHandler(key, 'float');
  ...
  ufloat(key, fallback) {
    ...
    if (value < 0)
      throw new this._errHandler(key, 'positive float');
  ...
  fixed(key, exp, fallback) {
    ...
    try {
      ...
      throw new this._errHandler(key, 'fixed number');
    }
  }
  ...
  ufixed(key, exp, fallback) {
    ...
    if (value < 0)
      throw new this._errHandler(key, 'positive fixed number');
  ...
  i8(key, fallback) {
    ...
    if (value < -0x80 || value > 0x7f)
      throw new this._errHandler(key, 'i8');
  ...
  i16(key, fallback) {
    ...
    if (value < -0x8000 || value > 0x7fff)
      throw new this._errHandler(key, 'i16');
  ...
  i32(key, fallback) {
    ...
    if ((value | 0) !== value)
      throw new this._errHandler(key, 'int32');
  ...
  u8(key, fallback) {
    ...
    if ((value & 0xff) !== value)
      throw new this._errHandler(key, 'uint8');
  ...
  u16(key, fallback) {
    ...
    if ((value & 0xffff) !== value)
      throw new this._errHandler(key, 'uint16');
  ...
  u32(key, fallback) {
    ...
    if ((value >>> 0) !== value)
      throw new this._errHandler(key, 'uint32');
  ...
  hash(key, fallback) {
    ...
    if (typeof value !== 'string') {
      if (!Buffer.isBuffer(value))
        throw new this._errHandler(key, 'hash');
    ...
    if (value.length !== 64)
      throw new this._errHandler(key, 'hex string');
  ...
  bool(key, fallback) {
    ...
    if (typeof value !== 'string') {
      if (typeof value !== 'boolean')
        throw new this._errHandler(key, 'boolean');
      return value;
    }
  ...
  buf(key, fallback, enc) {
    ...
    if (typeof value !== 'string') {
      if (!Buffer.isBuffer(value))
        throw new this._errHandler(key, 'buffer');
      return value;
    }
    ...
    if (data.length !== Buffer.byteLength(value, enc))
      throw new this._errHandler(key, `${enc} string`);
  ...
  array(key, fallback) {
    ...
    if (typeof value !== 'string') {
      if (!Array.isArray(value))
        throw new this._errHandler(key, 'array');
      return value;
    }
  ...
  obj(key, fallback) {
    ...
    if (typeof value !== 'object' || Array.isArray(value))
      throw new this._errHandler(key, 'object');
  ...
  func(key, fallback) {
    ...
    if (typeof value !== 'function')
      throw new this._errHandler(key, 'function');
   ...
   * @param {String} key
   * @param {Object?} fallback
   * @returns {Object|null} Decimal
   */
  decimal(key, fallback) {
    const value = this.float(key);
    ...
    try {
      ...
      throw new this._errHandler(key, 'decimal');
    }
  }
}
...
class ValidationError extends Error {
  constructor(key, type) {
    ...
    this.message = `${fmt(key)} must be a/an ${type}.`;
